...
const NutritionistLogin = () => {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("http://localhost:3000/nutrtionistlogin", {
        email: login.email,
        password: login.password
      });

      const { token, nutritionist } = response.data;
      // Store the token and user details in localStorage or sessionStorage
      // for authentication purposes
      localStorage.setItem('token', token);
      localStorage.setItem('user', JSON.stringify(nutritionist));

      // Redirect the user to the desired page
      // For example:
      navigate('/home_nutriHome');
    } catch (error) {
      console.error(error);
                ...
                <div className="row">
                  <div className="col-md-10 pt-5 mx-auto">
                    <div className="text-center">
                      <h2 className="text-bold">Nutritionist's Login</h2>
              ...
              {login.error && (
                <div className="col-md-12 text-center">
                  ...
                  <div className="col-md-12 text-center pt-2">
                    <h6 className="py-3" > Not a member? <Link to="/nutritionistSignup" className="btn text-danger"  >Sign Up</Link> </h6>
